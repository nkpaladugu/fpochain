...
contract FarmProducts {
    ...
    {
        require(
            product_.fpoaddress == msg.sender,
            "Only FPO can add"
        );
    ...
    function sell(
        ...
        if (Types.UserRole(party_.role) == Types.UserRole.Farmer) {
            productHistory[barcodeId_].supplier = userHistory_;
        } else if (Types.UserRole(party_.role) == Types.UserRole.Fpo) {
